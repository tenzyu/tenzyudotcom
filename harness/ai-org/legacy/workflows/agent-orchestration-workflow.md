...
Use `harness/ai-org/exec-plans/active/*.md` as the active workspace.
...
Only delegate plans from `harness/ai-org/exec-plans/active/*.md` that satisfy [exec-plan-contract](./exec-plan-contract.md) and have `execution-ready: true`.
...
`harness/ai-org/exec-plans/completed/*.md` is a work-log area, not the canonical rule source.
