...
Atelier is not an uncontrolled autonomous agent or a standalone agent
personality. In v1 it does not own the agent runtime. In later phases it may
host governed agent loops, but every autonomous action must pass through
Artifact Graph, Policy Engine, Task state, and Trace.  
...
For v1 Markdown-backed knowledge, the default workflow is:
...
In the v2 control-plane model, knowledge growth is broader. Artifact events,
curated control edits, checks, linters, hooks, CI, package scripts, code
structure, import graphs, failed runs, review comments, traces, and human
decisions may produce observed graph facts or candidate knowledge. Promotion to
durable Markdown is optional, not the only way knowledge enters Atelier.
...
- a generic chat-first coding UI or UI-only source of truth
...
Atelier may provide product-owner interaction surfaces, task dialogs, approval
dialogs, and graph-driven operation panels.
...
the ignored generated cache. The initial state root is:
...
Knowledge は **Markdown-backed 意味ノード**である。
frontmatter はその知識の**型・接続・選択条件**を宣言する索引兼型注釈である。

Markdown body is human-readable meaning. Atelier should not duplicate the full
meaning into frontmatter. Body interpretation must follow the deterministic-first
hierarchy: exact parsing, structural analysis, rule-based classification,
heuristics, cached semantic summaries, then LLM semantic interpretation only
when cheaper methods are insufficient.
...
`scan` must be read-only by default. It may write the materialized
`harness/atelier/graph.json` projection only through an explicit write mode once
the graph schema is stable.
...
## 13. Skills and Agent Adapters

Skills and adapters are control artifacts.

Some skills are generated, some are managed, some are curated, and some are
observed from external agent environments. They must not become an independent
hidden knowledge base, but curated edits are valid inputs to the Artifact Graph.

Atelier should observe skill edits, preserve lineage, infer affected knowledge
or controls when possible, and reconcile downstream context, roles, and tasks.
