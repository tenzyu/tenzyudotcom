...
export function PointersEditorClient({
  ...
  const handleFetchMetadata = async (catIdx: number, linkIdx: number) => {
    ...
    setLoadingPath(path)
    const metadata = await fetchUrlMetadataAction(link.url)
    setLoadingPath(null)
    ...
    setEntries((current) =>
      current.map((cat, ci) =>
        ci === catIdx
          ? {
              ...cat,
              links: cat.links.map((l, li) =>
                li === linkIdx
                  ? {
                      ...l,
                      title: {
                        ja: l.title.ja || metadata.title || '',
                        en: l.title.en || metadata.title || '',
                      },
                      description: {
                        ja: l.description.ja || metadata.description || '',
                        en: l.description.en || metadata.description || '',
                      },
                    }
                  : l,
              ),
