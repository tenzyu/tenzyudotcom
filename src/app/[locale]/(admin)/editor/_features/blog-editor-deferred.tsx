'use client'

import { useEffect, useState } from 'react'
import type { MDXData } from '@/app/[locale]/(main)/blog/_features/blog.domain'
import { BlogEditor } from './blog-editor'
import { Loader2 } from 'lucide-react'

type BlogEditorDeferredProps = {
  locale: string
}

export function BlogEditorDeferred({ locale }: BlogEditorDeferredProps) {
  const [state, setState] = useState<{ posts: MDXData[] } | null>(null)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    async function load() {
      try {
        const res = await fetch('/api/editor/blog')
        if (!res.ok) throw new Error('Failed to load')
        const data = await res.json()
        setState({
          posts: data.collection,
        })
      } catch (_e) {
        setError('Failed to load Blog collection')
      }
    }
    load()
  }, [])

  if (error) return <div className="text-destructive text-sm">{error}</div>
  if (!state) return <div className="flex justify-center p-8"><Loader2 className="animate-spin text-muted-foreground" /></div>

  return (
    <BlogEditor
      locale={locale}
      posts={state.posts}
    />
  )
}
