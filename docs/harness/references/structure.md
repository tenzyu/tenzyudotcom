...
ただし `src/components/shell`, `src/components/site-ui`, `src/config` は sanctioned exception である。
...
- route 内で共有する transform / loader / builder は `<route>/_features/<name>.ts` を第一候補にする
