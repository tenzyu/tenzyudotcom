...
export async function loadPuzzlesState() {
  return loadJsonCollection(
    PUZZLES_STORAGE_PATH,
    parsePuzzleSourceCategories,
    () => [] as readonly PuzzleCategory[],
  )
}

export async function savePuzzlesState(
  rawJson: string,
...
export function makePuzzlesRepository() {
  return new PuzzlesStorageRepository()
}
