    ...
    <>
      ...
      <Card variant="quiet" className="gap-0 py-0">
        <CardContent className="space-y-3 p-5">
          <h2 className="text-lg font-semibold tracking-tight">{content.overview.title}</h2>
          <p className="text-muted-foreground text-sm leading-6">{content.overview.description}</p>
          <div className="flex flex-wrap items-center gap-2 text-sm">
            <span className="font-medium">{content.overview.futureTitle}</span>
            {futureItems.map((item) => (
              <Badge key={item.value} variant="secondary">{item.value}</Badge>
            ))}
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {tools.map((tool) => {
          ...
          return (
            <Card key={tool.href.value} variant="interactive" className="h-full">
              ...
              <CardContent className="flex flex-col gap-3 text-sm">
                <OtakuAside label={content.labels.comment.value}>{tool.note}</OtakuAside>
      ...
      <Card variant="quiet" className="gap-0 py-0">
        <CardContent className="flex flex-wrap items-center gap-3 p-5 text-sm">
          <p className="text-muted-foreground leading-6">{content.support.description}</p>
          <KoFiLink label={content.support.supportLabel.value} />
          <Button asChild variant="ghost" size="sm">
            <Link href={getLocalizedUrl('/blog', locale)}>{content.support.readBlogLabel}</Link>
          </Button>
        </CardContent>
      </Card>
    </>
