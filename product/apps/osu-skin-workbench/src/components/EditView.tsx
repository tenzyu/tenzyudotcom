...
export function EditView(props: Props) {
  ...
  const rows = useMemo(
    ...
    [
      ...
      props.category,
      props.filter,
      props.meaningfulOnly,
      props.collapseStable,
      requiredLevel,
    ]
  )

  const activeScope = props.matrix.rows.find((row) => row.scope === props.scope)
    ?.taxonomy.scope
  const activeCategory = props.matrix.rows.find(
    (row) => row.scope === props.scope && row.category === props.category
  )?.taxonomy.category
  const sourceColumns = props.matrix.columns.filter(
    (column) => column.kind === 'source'
  )
  const activeSourceColumn =
    sourceColumns.find((column) => column.id === props.selectedSourceId) ??
    sourceColumns[0] ??
    null
  ...
  const sourcePresent = activeSourceColumn
    ? rows.filter(
        (row) => !sourceCellFor(row, activeSourceColumn.id).cell.missing
      ).length
    : 0
  ...
  return (
    <section className={`editView density-${density}`}>
        ...
        <CardContent className="filterBarContent">
          ...
          <div className="fieldStack compactField">
            ...
            <NativeSelect
              ...
              value={requiredLevel}
              onChange={(event) =>
                setRequiredLevel(event.target.value as RequiredLevelFilter)
              }
            >
          ...
          <div className="toggleStack">
            <label className="checkboxLine">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  props.onMeaningfulOnly(checked === true)
                }
              />
            ...
            <label className="checkboxLine">
              <Checkbox
                ...
                onCheckedChange={(checked) =>
                  props.onCollapseStable(checked === true)
                }
              />
      ...
      <div className="actionStrip">
        ...
        <div className="actionStripButtons">
          <Button
            ...
            size="sm"
            onClick={() =>
              selectRows(rows.filter((row) => row.cells.project.missing))
            }
          >
            Select missing
          </Button>
          <Button
            type="button"
            size="sm"
            onClick={() => selectRows(rows.filter(rowHasWarning))}
          >
            Select warnings
          </Button>
          <Button
          ...
          <Button
            ...
            disabled={!activeSourceColumn || selectedVisibleCount === 0}
          >
      ...
      <div
        className={`validationSummary ${warnings.total ? 'statusWarning' : 'statusQuiet'}`}
      >
        <div>
          <strong>{warnings.total} warnings</strong>
          <span>
            HD only {warnings.hdOnly} · Missing {warnings.missing} · Scope{' '}
            {activeScope?.label ?? props.scope} /{' '}
            {activeCategory?.label ?? props.category}
          </span>
        </div>
        <Button
          type="button"
              ...
              rows.map((row) => {
                ...
                return (
                  <CompareAssetCard
                    ...
                    onDelete={() =>
                      props.onDeleteProjectRow(pathsFromCell(row.cells.project))
                    }
                    onRestore={() =>
                      void props.onCopySourceRow({
                        sourceId: activeSourceColumn.id,
