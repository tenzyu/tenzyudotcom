...
async function validateReachability(designDocsDir: string) {
  ...
  for (const file of allMdFiles) {
    ...
    if (!reachedFiles.has(file)) {
      console.error(`\x1b[31m[ERROR]\x1b[0m Orphaned rule document (unreachable from design-docs/AGENTS.md): ${path.relative(process.cwd(), file)}`)
      hasErrors = true
    }
  }
...
async function run() {
  const designDocsDir = path.resolve(process.cwd(), 'docs/design-docs')
  const agentsMd = path.join(designDocsDir, 'AGENTS.md')
  let hasErrors = false
